...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (isMobile) {
        // Add device orientation event for mobile devices
        if (window.DeviceOrientationEvent) {
            // Request permission for iOS 13+ devices
            if (typeof DeviceOrientationEvent.requestPermission === 'function') {
                // Create a button to request permission
                const permissionButton = document.createElement('button');
                permissionButton.innerText = 'Enable Motion Effects';
                permissionButton.classList.add('permission-button');
        ...
        // Add mouse move event for desktop devices
...
function createInteractiveBackgroundStars() {
    ...
    // Store interaction position (mouse or device orientation)
    let interactionX = 0;
    let interactionY = 0;
    let isInteracting = false;
    let interactionTimeout;
    
    // Store device orientation values
    let deviceBeta = 0;  // -180 to 180 (tilt front/back)
    let deviceGamma = 0; // -90 to 90 (tilt left/right)
    
    // Update mouse position for desktop
    ...
    function animate() {
        ...
        if (isInteracting) {
            const gradient = ctx.createRadialGradient(interactionX, interactionY, 0, interactionX, interactionY, 200);
        ...
        stars.forEach(star => {
            ...
            const dx = interactionX - star.x;
            const dy = interactionY - star.y;
            ...
            // Interaction effect - stronger effect
            if (isInteracting && distance < 300) {
...
// Handle mouse movement for star interaction on desktop
function handleMouseMove(e) {
    ...
    // Update the global interaction coordinates for the background stars
    interactionX = mouseX;
    interactionY = mouseY;
    isInteracting = true;
...
// Handle device orientation for star interaction on mobile
function handleDeviceOrientation(e) {
    // Get device orientation data
    const beta = e.beta;  // -180 to 180 (tilt front/back)
    const gamma = e.gamma; // -90 to 90 (tilt left/right)
    ...
    // Map the tilt values to screen coordinates
    const maxTilt = 45; // Maximum tilt angle to consider (degrees)
    
    // Normalize tilt values to a range of 0-1
    ...
    interactionX = window.innerWidth * (0.5 + normalizedGamma * 0.5); // Map -1,1 to 0,width
    interactionY = window.innerHeight * (0.5 + normalizedBeta * 0.5); // Map -1,1 to 0,height
    isInteracting = true;
    ...
    // Add a visual cursor effect at the calculated position
    addCursorEffect(interactionX, interactionY);
    ...
    stars.forEach(star => {
        ...
        const distance = Math.sqrt(Math.pow(interactionX - starX, 2) + Math.pow(interactionY - starY, 2));
        
        // If interaction point is close to star
        if (distance < 200) {
            const scale = 1 + (200 - distance) / 100 * 2; // Exaggerated scale effect
            const currentRadius = parseFloat(star.getAttribute('r'));
            const originalRadius = parseFloat(star.dataset.originalRadius || currentRadius);
            ...
            // Scale the star based on proximity
            star.setAttribute('r', originalRadius * scale);
